...
using (var scope = app.Services.CreateScope())
{
    ...
    if (!db.Categories.Any())
    {
        db.Categories.AddRange(
            new PetShop.Models.Category { TenDanhMuc = "Thức ăn chó", MoTa = "Các loại thức ăn dành cho chó" },
            new PetShop.Models.Category { TenDanhMuc = "Thức ăn mèo", MoTa = "Các loại thức ăn dành cho mèo" },
            new PetShop.Models.Category { TenDanhMuc = "Phụ kiện chó", MoTa = "Vòng cổ, dây dắt, áo..." },
            new PetShop.Models.Category { TenDanhMuc = "Phụ kiện mèo", MoTa = "Nhà mèo, đồ chơi, cát vệ sinh..." }
        );
        db.SaveChanges();
    }
    ...
    if (!db.Products.Any())
    {
        ...
        db.Products.AddRange(
            new PetShop.Models.Product { TenSanPham = "Thức ăn hạt Royal Canin cho chó 1kg", Gia = 185000, SoLuongKho = 50, CategoryId = cat1, MoTa = "Thức ăn hạt cao cấp cho chó trưởng thành" },
            new PetShop.Models.Product { TenSanPham = "Pate Whiskas cho chó 400g", Gia = 45000, SoLuongKho = 100, CategoryId = cat1, MoTa = "Pate thơm ngon bổ dưỡng cho chó" },
            new PetShop.Models.Product { TenSanPham = "Thức ăn hạt Me-O cho mèo 1.2kg", Gia = 95000, SoLuongKho = 80, CategoryId = cat2, MoTa = "Hạt khô đầy đủ dinh dưỡng cho mèo" },
            new PetShop.Models.Product { TenSanPham = "Pate Whiskas cho mèo 85g", Gia = 18000, SoLuongKho = 200, CategoryId = cat2, MoTa = "Pate mềm thơm cho mèo mọi lứa tuổi" },
            new PetShop.Models.Product { TenSanPham = "Vòng cổ da cho chó size M", Gia = 75000, SoLuongKho = 30, CategoryId = cat3, MoTa = "Vòng cổ da bền đẹp cho chó cỡ vừa" },
            new PetShop.Models.Product { TenSanPham = "Dây dắt chó 1.5m", Gia = 55000, SoLuongKho = 40, CategoryId = cat3, MoTa = "Dây dắt chắc chắn, thoải mái khi dắt chó đi dạo" },
            new PetShop.Models.Product { TenSanPham = "Nhà mèo lông xù size L", Gia = 320000, SoLuongKho = 15, CategoryId = cat4, MoTa = "Nhà lông ấm áp, mèo thích nằm" },
            new PetShop.Models.Product { TenSanPham = "Cần câu đồ chơi cho mèo", Gia = 35000, SoLuongKho = 60, CategoryId = cat4, MoTa = "Đồ chơi cần câu lông vũ kích thích mèo vận động" }
        );
        db.SaveChanges();
    ...
    if (!db.Reviews.Any())
    {
        ...
        // Nếu chưa có user khách hàng thì tạo vài user mẫu
        if (!users.Any())
        {
            db.Users.AddRange(
            ...
            db.SaveChanges();
            users = db.Users.Where(u => u.VaiTro == "KhachHang").ToList();
        }

        // Nội dung review mẫu theo số sao
        ...
        foreach (var product in products)
        {
            ...
            for (int i = 0; i < shuffledUsers.Count; i++)
            {
                ...
                var contents = reviewContents[soSao];
                string noiDung = contents[random.Next(contents.Count)];
    ...
    // Seed IsHot / IsBanChay
    var productsToUpdate = db.Products.ToList();
    if (productsToUpdate.Any(p => !p.IsHot && !p.IsBanChay))
    {
        // Hàng Hot — chọn 6 sản phẩm
        var hotNames = new[]
        {
        "Thức ăn hạt Royal Canin cho chó 1kg",
        "Thức ăn hạt Me-O cho mèo 1.2kg",
        "Nhà mèo lông xù size L",
        "Thức ăn hạt Royal Canin cho mèo con 400g",
        "Thức ăn hạt Ganador cho chó lớn 3kg",
        "Trụ cào móng cho mèo 60cm"
    };

        // Bán chạy — chọn 8 sản phẩm
        var banChayNames = new[]
        {
        "Pate Whiskas cho chó 400g",
        "Pate Whiskas cho mèo 85g",
        "Snack mèo Temptations vị gà 85g",
        "Bánh thưởng Treats cho chó 200g",
        "Snack que tươi Inaba Churu cho mèo",
        "Cần câu đồ chơi cho mèo",
        "Dây dắt chó 1.5m",
        "Lược chải lông mèo tự làm sạch"
    };

        foreach (var p in productsToUpdate)
        {
            if (hotNames.Contains(p.TenSanPham))
                p.IsHot = true;

            if (banChayNames.Contains(p.TenSanPham))
                p.IsBanChay = true;
        }
